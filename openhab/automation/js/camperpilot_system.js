rules.JSRule({
  ...
  triggers: [
    triggers.ItemCommandTrigger('System_Shutdown', 'ON')
  ],
  execute: (event) => {
    console.warn('Shutdown wurde über openHAB angefordert');

    items.getItem('System_Shutdown').postUpdate('OFF');
...
rules.JSRule({
  ...
  triggers: [
    triggers.ItemCommandTrigger('System_Reboot', 'ON')
  ],
  execute: (event) => {
    console.warn('Reboot wurde über openHAB angefordert');

    items.getItem('System_Reboot').postUpdate('OFF');
